...
RSpec.describe VeterinaryOffice, type: :model do
  ...
  let!(:vet_office_1) { VeterinaryOffice.create(name: 'Best Vets', boarding_services: true, max_patient_capacity: 20) }
  let!(:vet_office_2) { VeterinaryOffice.create(name: 'Vets R Us', boarding_services: true, max_patient_capacity: 20) }
  let!(:not_on_call_vet) { Veterinarian.create(name: 'Sam', review_rating: 10, on_call: false, veterinary_office_id: vet_office_1.id) }
  let!(:vet_1) { Veterinarian.create(name: 'Taylor', review_rating: 10, on_call: true, veterinary_office_id: vet_office_1.id) }
  let!(:vet_2) { Veterinarian.create(name: 'Jim', review_rating: 8, on_call: true, veterinary_office_id: vet_office_1.id) }
  let!(:vet_3) { Veterinarian.create(name: 'Sarah', review_rating: 9, on_call: true, veterinary_office_id: vet_office_2.id) }

  describe 'class methods' do
    describe '::search' do
      it 'returns partial matches' do
        expect(VeterinaryOffice.search('Vets')).to eq([vet_office_1, vet_office_2])
      end
    end
