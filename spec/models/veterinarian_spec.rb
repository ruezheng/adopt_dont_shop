...
RSpec.describe Veterinarian, type: :model do
  ...
  let!(:vet_1) { vet_office.veterinarians.create(name: 'Taylor', review_rating: 10, on_call: true) }
  let!(:vet_2) { vet_office.veterinarians.create(name: 'Tanya', review_rating: 9, on_call: true) }
  let!(:vet_3) { vet_office.veterinarians.create(name: 'Jim', review_rating: 8, on_call: true) }
  let!(:not_on_call_vet) { vet_office.veterinarians.create(name: 'Sam', review_rating: 10, on_call: false) }
