...
RSpec.describe 'the pets index' do
  ...
  let!(:pet_1) { Pet.create(adoptable: true, age: 7, breed: 'sphynx', name: 'Bare-y Manilow', shelter_id: shelter.id) }
  let!(:pet_2) { Pet.create(adoptable: true, age: 3, breed: 'domestic pig', name: 'Babe', shelter_id: shelter.id) }
  let!(:pet_3) { Pet.create(adoptable: true, age: 4, breed: 'chihuahua', name: 'Elle', shelter_id: shelter.id) }
  ...
  it 'has link to create new application' do
    visit "/pets"
